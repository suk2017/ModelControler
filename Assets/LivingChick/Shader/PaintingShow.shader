	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (appdata v)
			{
				...
				float3 viewDir = normalize( mul(unity_WorldToObject, float4(_WorldSpaceCameraPos.xyz, 1)).xyz - v.vertex);//获取视角方向 指向摄像机
				o.vdotn = dot(normalize(viewDir),v.normal);//计算成边度
